...
class CartPageState extends State<CartPage> {
  ...
  Widget _cardsTab(state) {
    _addCard(cardToken) async {
      ...
      await http.put(Uri.parse('http://localhost:1337/users/${user.id}'),
          body: {"card_token": cardToken},
          headers: {"Authorization": "Bearer ${user.jwt}"});
      // associate cardToken (added card) with Stripe customer (POST /card/add)
      http.Response response = await http.post(Uri.parse('http://localhost:1337/card/add'),
  ...
  Future _showCheckoutDialog(state) {
    return showDialog(
      ...
      _checkoutCartProducts() async {
        ...
        await http.post(Uri.parse('http://localhost:1337/orders'), body: {
